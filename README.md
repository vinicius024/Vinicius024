# Olá, eu sou Vinicius de Freitas e Silva 👋

### Desenvolvedor Frontend & UX/UI Designer

Sou apaixonado por tecnologia, design e criação de interfaces digitais.  
Atuo unindo **UX/UI Design, Frontend e Design Gráfico**, criando experiências visuais que vão desde a prototipação no **Figma** até a implementação com **React, Next.js e TypeScript**.

Atualmente curso **Análise e Desenvolvimento de Sistemas** e venho desenvolvendo projetos com foco em interfaces responsivas, usabilidade, código limpo e boas práticas de desenvolvimento.

---

## Sobre mim

- Desenvolvedor Frontend com experiência em **React.js, Next.js e TypeScript**
- UX/UI Designer com foco em **prototipação, interfaces responsivas e design-to-code**
- Experiência com **Figma, TailwindCSS e Styled-components**
- Conhecimento em backend com **Node.js, Express, Prisma e APIs REST**
- Experiência com projetos reais, sistemas internos, landing pages e dashboards
- Também tenho base em **Design Gráfico**, criação de logos, identidade visual e artes digitais

---

## Tecnologias e ferramentas

### Frontend

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-111111?style=for-the-badge&logo=nextdotjs&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-1E1E1E?style=for-the-badge&logo=typescript&logoColor=3178C6)
![JavaScript](https://img.shields.io/badge/JavaScript-1E1E1E?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![HTML5](https://img.shields.io/badge/HTML5-1E1E1E?style=for-the-badge&logo=html5&logoColor=E34F26)
![CSS3](https://img.shields.io/badge/CSS3-1E1E1E?style=for-the-badge&logo=css3&logoColor=1572B6)

### UI/UX Design

![Figma](https://img.shields.io/badge/Figma-1E1E1E?style=for-the-badge&logo=figma&logoColor=F24E1E)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-1E1E1E?style=for-the-badge&logo=tailwindcss&logoColor=38B2AC)
![Styled Components](https://img.shields.io/badge/Styled--Components-1E1E1E?style=for-the-badge&logo=styledcomponents&logoColor=DB7093)

### Backend e Banco de Dados

![Node.js](https://img.shields.io/badge/Node.js-1E1E1E?style=for-the-badge&logo=nodedotjs&logoColor=339933)
![Express](https://img.shields.io/badge/Express-1E1E1E?style=for-the-badge&logo=express&logoColor=white)
![Prisma](https://img.shields.io/badge/Prisma-1E1E1E?style=for-the-badge&logo=prisma&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-1E1E1E?style=for-the-badge&logo=mongodb&logoColor=47A248)

### Ferramentas

![Git](https://img.shields.io/badge/Git-1E1E1E?style=for-the-badge&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-1E1E1E?style=for-the-badge&logo=github&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-1E1E1E?style=for-the-badge&logo=vercel&logoColor=white)

---

## Experiência

Já atuei em projetos envolvendo:

- Desenvolvimento de interfaces e dashboards em **React e TypeScript**
- Integração com **APIs REST**
- Criação de telas, fluxos e protótipos de alta fidelidade no **Figma**
- Desenvolvimento de landing pages e sites institucionais
- Deploy de aplicações na **Vercel**
- Liderança técnica em equipe multidisciplinar com UX/UI, Frontend e Backend
- Criação de identidade visual, logos e materiais gráficos

---

## Atualmente estudando

- Aprofundamento em **Frontend moderno**
- Boas práticas com **React, Next.js e TypeScript**
- UX/UI Design aplicado a produtos digitais
- Arquitetura de projetos frontend
- Backend com Node.js e banco de dados

---

## GitHub Stats

<div align="left">

![Vinicius GitHub stats](https://github-readme-stats.vercel.app/api?username=Vinicius024&show_icons=true&theme=tokyonight&hide_border=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Vinicius024&layout=compact&theme=tokyonight&hide_border=true)

</div>

---

## Contato

📧 E-mail: **viniciusfreitassilva2030@gmail.com**  
💼 LinkedIn: [**coloque-seu-link-aqui** ](https://www.linkedin.com/in/vinicius024/) 
🌐 Portfólio: **coloque-seu-link-aqui**

---

> “Design não é só como algo parece. É como funciona.”
